...
const APPS = [
  ...
  },
  {
    ...
    why: {
      pain: '성격유형 콘텐츠는 이미 넘칩니다. 그런데 대부분 "당신은 OO형입니다"에서 끝납니다. 결과를 한 번 보고 캡처해서 공유하면 그 앱을 다시 열 이유가 사라집니다. 만드는 쪽에서 보면, 유저를 데려오는 비용은 똑같이 드는데 한 번 쓰고 끝나는 구조입니다.',
      gap: '유형 진단이 계속 쓰이려면 결과가 끝이 아니라 입구가 되어야 했습니다. 그래서 유형을 진단하는 앱이 아니라, 유형에 따라 답이 갈리는 문제를 계속 푸는 앱으로 형식을 바꿨습니다. 퀴즈는 콘텐츠만 채우면 무한히 반복되는 포맷이라, 재방문 이유를 콘텐츠 쪽에 둘 수 있습니다.',
      question: '한 번 보고 닫는 결과가 아니라, 계속 돌아올 이유를 만들 수 있을까?',
    },
    decisions: [
      {
        t: '게임으로 만들지 않기로 한 결정',
  ...
  },
  {
    ...
    why: {
      pain: '반려인은 하루에 몇 번씩 "지금 나가도 되나"를 판단합니다. 그런데 그 판단에 필요한 정보가 앱 두 개에 흩어져 있습니다. 날씨 앱에서 기온과 강수를 보고, 미세먼지 앱에서 대기질을 따로 확인해야 합니다.',
      gap: '더 큰 문제는 두 앱 모두 원자료만 준다는 것이었습니다. "체감 31도, PM2.5 55"를 보고 나가도 되는지는 사용자가 스스로 결론을 내야 합니다. 게다가 기준이 사람 기준입니다 — 털이 있고 지면에 가까운 반려동물에게 아스팔트 온도가 어떤 의미인지는 어디에도 없습니다. 필요한 건 데이터가 아니라 결론이었습니다.',
      question: '숫자 여러 개 말고, 나가도 되는지 아닌지만 알려줄 수 없을까?',
    },
    decisions: [
      {
  ...
  },
  {
    ...
    why: {
      pain: '소비 성향을 알려주는 서비스는 대개 가계부입니다. 몇 달치 지출을 쌓아야 결과가 나오고, 그 몇 달을 버티는 사람이 거의 없습니다. 반대로 밸런스 게임은 진입은 쉬운데 결과가 "당신은 짜장면파"에서 끝나 남는 게 없습니다.',
      gap: '진입 장벽이 낮은 쪽은 알려주는 게 없고, 알려주는 쪽은 진입이 어렵습니다. 그 사이를 노렸습니다. 형식은 밸런스 게임 그대로 두되, 문항을 소비 상황으로 짜서 8문항만 고르면 유형까지 나오게 했습니다. 기록 없이, 재미로 고른 선택만으로 결과를 만드는 구조입니다.',
  ...
  },
  {
    ...
    why: {
      pain: '사무실에서 커피 내기를 할 때 쓰는 도구는 대개 사다리타기입니다. 결과는 정확하지만 뽑는 순간이 재미없고, 광고가 결과 화면을 덮거나 정말 공정한지 확인할 방법이 없는 앱이 섞여 있습니다. 내기는 결과에 승복해야 끝나는데, 승복이 안 되면 도구를 다시 안 씁니다.',
      gap: '이 도구에 필요한 건 두 가지였습니다. 공정하다고 믿을 수 있을 것, 그리고 결과가 나오기까지가 재미있을 것. 대부분의 서비스가 둘 중 하나를 포기하는데 — 재미를 위해 확률을 비틀거나, 공정성을 위해 연출을 없애거나 — 이 둘은 사실 서로 다른 층위라 동시에 가질 수 있다고 봤습니다.',
